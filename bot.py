...
(WAITING_TYPE,
 ...
 WAITING_MENU_WHO, WAITING_MENU_ITEM) = range(12)
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t == "🍽 Меню недели":
        await update.message.reply_text(
            "Кто выбирает?", reply_markup=MENU_WHO_KB)
...
# ── МЕНЮ НЕДЕЛИ ───────────────────────────────────────────────────────────
def _menu_build_item_kb(can_finish: bool):
    rows = [["✅ Беру", "➡️ Дальше"]]
    if can_finish:
        rows.append(["⏭ Готово с категорией"])
    rows.append(["❌ Отмена"])
    return ReplyKeyboardMarkup(rows, resize_keyboard=True)

def _menu_render_item(context: ContextTypes.DEFAULT_TYPE):
    m = context.user_data["menu"]
    cat = MENU_CATEGORIES[m["cat"]]
    items = cat["items"]
    idx = m["item"]
    chosen_here = len(m["selected"].get(cat["key"], []))
    can_finish = chosen_here >= cat["min"]
    text = (f"{cat['name']} — выбрать {cat['min']}"
            + (f"–{cat['max']}" if cat['max'] != cat['min'] else "")
            + f"\nВыбрано: {chosen_here}/{cat['max']} · вариант {idx + 1} из {len(items)}\n\n"
            + items[idx])
    return text, _menu_build_item_kb(can_finish)
...
async def _menu_advance_category(update, context):
    ...
    if m["cat"] >= len(MENU_CATEGORIES):
        ...
        menu_done[who] = True
        await update.message.reply_text(
            f"Спасибо, {who}! Твой выбор сохранён 🎉", reply_markup=MAIN_KB)
        if all(menu_done.values()):
            await update.message.reply_text(
                _menu_build_final(), parse_mode="Markdown", reply_markup=MAIN_KB)
        else:
            waiting_for = [p for p in MENU_PEOPLE if not menu_done[p]]
            await update.message.reply_text(
                f"Жду ещё: {', '.join(waiting_for)} 👀", reply_markup=MAIN_KB)
        context.user_data.pop("menu", None)
        return ConversationHandler.END
    text, kb = _menu_render_item(context)
...
async def pick_menu_who(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t not in MENU_PEOPLE:
        ...
        return WAITING_MENU_WHO
    context.user_data["menu"] = {"who": t, "cat": 0, "item": 0, "selected": {}}
    text, kb = _menu_render_item(context)
    await update.message.reply_text(
        f"Погнали, {t}! Листаем блюда 👇\n\n{text}", reply_markup=kb)
    return WAITING_MENU_ITEM
...
async def menu_item_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not m:
        await update.message.reply_text("Нажми /start, чтобы начать заново.", reply_markup=MAIN_KB)
        return ConversationHandler.END
    ...
    if t == "✅ Беру":
        chosen = m["selected"].setdefault(cat["key"], [])
        chosen.append(items[m["item"]])
        if cat.get("always_extra") and cat["always_extra"] not in chosen:
            chosen.append(cat["always_extra"])
        if len(chosen) - (1 if cat.get("always_extra") else 0) >= cat["max"]:
    ...
    if t == "⏭ Готово с категорией":
        chosen_here = len(m["selected"].get(cat["key"], []))
        if chosen_here < cat["min"]:
            await update.message.reply_text(
                f"Нужно выбрать хотя бы {cat['min']} — выбери ещё 👇")
            text, kb = _menu_render_item(context)
            await update.message.reply_text(text, reply_markup=kb)
            return WAITING_MENU_ITEM
        return await _menu_advance_category(update, context)
