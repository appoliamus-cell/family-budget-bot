...
# СТРОКИ В ТАБЛИЦЕ (Google Sheets, 1-индексация)
ROWS = {
...
# ИТОГО РАСХОДЫ = строка 55, ОСТАТОК = строка 56
ROW_INC_TOT = 10   # ИТОГО ДОХОДЫ
ROW_EXP_TOT = 55   # ИТОГО РАСХОДЫ
ROW_BALANCE = 56   # ОСТАТОК
 
BTN_MAP = {k: k for k in ROWS.keys()}
# убираем emoji-дубли для кнопок
BTN_MAP.update({
...
async def pick_cat_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"*{cat}* — сколько?", parse_mode="Markdown", reply_markup=CANCEL_KB)
...
async def enter_amount_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Введи только число, например *320*", parse_mode="Markdown", reply_markup=CANCEL_KB)
        return WAITING_AMOUNT_ADD
...
async def pick_cat_del(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cat:
        ...
        return WAITING_CAT_DEL
    context.user_data["cat"] = cat
    await update.message.reply_text(f"*{cat}* — сколько удалить?", parse_mode="Markdown", reply_markup=CANCEL_KB)
...
async def pick_income_who(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"*{info[1]}* — сколько?", parse_mode="Markdown", reply_markup=CANCEL_KB)
...
async def cmd_per_day(update: Update):
    try:
        ...
        inc = get_val(ws, ROW_INC_TOT, 4)
        exp = get_val(ws, ROW_EXP_TOT, 4)
        rest_total = inc - exp
...
async def cmd_итого(update: Update):
    try:
        ws = get_sheet()
        inc_p = get_val(ws, ROW_INC_TOT, 3)
        inc_f = get_val(ws, ROW_INC_TOT, 4)
        exp_p = get_val(ws, ROW_EXP_TOT, 3)
        exp_f = get_val(ws, ROW_EXP_TOT, 4)
        rest_f = inc_f - exp_f
